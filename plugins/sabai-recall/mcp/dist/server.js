...
async function recallFetchV2(endpoint, options = {}) {
    const config = getConfig();
...
export function createServer() {
    ...
    server.tool("recall_list_calendars", "List all calendars connected to Recall.ai. Returns calendar IDs needed for other calendar operations.", {}, async () => {
        const response = await recallFetchV2("/calendars/");
    ...
    server.tool("recall_list_calendar_events", "List upcoming calendar events. Use this to find meetings that can be recorded. Returns events with meeting URLs.", {
        ...
        const response = await recallFetchV2(endpoint);
    ...
    server.tool("recall_get_current_meeting", "Get the current meeting (happening now) or the next upcoming meeting from connected calendars. Perfect for /join without a URL.", {}, async () => {
        const now = new Date();
        // Look for events in the next 24 hours
        const tomorrow = new Date(now.getTime() + 24 * 60 * 60 * 1000);
        const response = await recallFetchV2(`/calendar-events/?start_time__gte=${encodeURIComponent(now.toISOString())}&start_time__lte=${encodeURIComponent(tomorrow.toISOString())}&is_deleted=false&limit=10`);
        // Also check for events that started recently (within last 2 hours) and may still be ongoing
        const twoHoursAgo = new Date(now.getTime() - 2 * 60 * 60 * 1000);
        const ongoingResponse = await recallFetchV2(`/calendar-events/?start_time__gte=${encodeURIComponent(twoHoursAgo.toISOString())}&start_time__lte=${encodeURIComponent(now.toISOString())}&is_deleted=false&limit=10`);
    ...
    server.tool("recall_schedule_bot_for_event", "Schedule a Recall bot to join a specific calendar event. The bot will automatically join when the meeting starts.", {
        ...
        const event = await recallFetchV2(`/calendar-events/${args.event_id}/bot/`, {
            method: "POST",
            body: JSON.stringify(requestBody),
        });
        return {
