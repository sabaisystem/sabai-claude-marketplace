...
import { listAccounts, getActiveAccount, switchAccount, addAccount, removeAccount, } from "./account-manager.js";
import { getConsentUrl } from "./auth.js";
import { getUserInfo, listEnvelopes, getEnvelope, getEnvelopeDocuments, getEnvelopeRecipients, } from "./docusign-client.js";
const server = new Server({ name: "sabai-docusign", version: "1.0.0" }, { capabilities: { tools: {} } });
// ============ TOOL DEFINITIONS ============
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        },
        {
            ...
            description: "Add a new DocuSign account profile for JWT authentication. Requires Integration Key, User ID, Account ID, and either the RSA private key content (PEM) or a file path to the key.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "List DocuSign envelopes (agreements) with optional filters. Returns envelope ID, subject, status, and recipient info.",
            inputSchema: {
                type: "object",
                properties: {
                    status: {
                        type: "string",
                        description: "Filter by status: created, sent, delivered, signed, completed, declined, voided, deleted. Comma-separate for multiple.",
                    },
                    fromDate: {
                        type: "string",
        ...
        },
        {
            ...
            description: "Get detailed information about a specific DocuSign envelope including recipients, documents, and status",
            inputSchema: {
                type: "object",
                properties: {
...
function requireActiveAccount() {
    ...
    if (!account) {
        throw new Error("No DocuSign account configured.\n\n" +
    ...
    try {
        switch (name) {
            ...
            case "docusign_switch_account": {
                const account = switchAccount(args?.name);
                return {
                    content: [{
                            type: "text",
            ...
            case "docusign_add_account": {
                addAccount({
                    ...
                    environment: args?.environment || "demo",
                });
                const account = getActiveAccount();
                ...
                return {
                    content: [{
                            type: "text",
                            text: `Account "${args?.name}" added successfully.\n\n` +
            ...
            case "docusign_get_envelope": {
                const account = requireActiveAccount();
                const envelope = await getEnvelope(account, args?.envelopeId);
                return {
            ...
            case "docusign_get_documents": {
                const account = requireActiveAccount();
                const docs = await getEnvelopeDocuments(account, args?.envelopeId);
                return {
            ...
            case "docusign_get_recipients": {
                const account = requireActiveAccount();
                const recipients = await getEnvelopeRecipients(account, args?.envelopeId);
                return {
